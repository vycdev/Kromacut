...
Run `npm run benchmark:incremental-exact` to compare full reconstruction with incremental optical-prefix palette construction for all 109,600 ordered no-repeat subsets in the calibrated eight-filament K-logo fixture. The benchmark requires exact palette, semantic evaluation, scalar score, winner, and tie-break equivalence and also exercises changed process fingerprints, first-layer height, maximum height, transition opacity, and fitted substrate interactions.
