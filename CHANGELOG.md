...
- **Frozen diagnostic print bundles** - Added an offline developer workflow for numbered thickness strips with full-profile predictions recorded before printing, complete backing recipes, repeat pads and known Matrix controls. Bundles contain separate physical-material 3MFs, maps, instructions, profile/model/source snapshots and integrity hashes, with serialized geometry, layer-stack and material regression checks. Predictions use actual physical layers for Matrix lookup while retaining compact runs in the recipe map. A separate post-print replay compares current predictions against preserved bundles, verifies all original hashes and physical recipes, and pins originally automatic foundation sizes to their printed thicknesses. These tools leave calibration and saved experiments unchanged.

- **Offline Matrix prediction validation** - Added a developer-run benchmark for exported filament profiles and desktop diagnostic traces that refits the Matrix prediction pipeline using training-only evidence, then scores withheld recipes, unseen filament pairs, and whole boards/photos when available. JSON and Markdown reports separate fixed-prior, fitted-optics, and full empirical predictions, retain worst cases and support details, and disclose fixed wedge priors, excluded Palette Proof evidence, and shared-photo limitations. Validation preserves physical recipe layers for empirical lookup while grouping identical recipes by compact material runs. Leakage and runtime-agreement regression tests accompany this read-only workflow.

- **Desktop Auto-paint diagnostic recording** - Desktop Settings can opt into one flushed JSONL trace per new Auto-paint calculation and open the trace folder directly. Partial files retain bounded progress, cancellation, and error events; completed files add the exact filament/calibration snapshot with Quick-versus-refined HD curves and observed/predicted wedge and merge reads, appearance-fit gates, per-transition optical provenance, progressive repeat-tier decisions, final physical stack, every target-to-printable-candidate Delta E comparison, prediction confidence, and normalized empirical or local evidence contributions without storing the uploaded source image. Recording stays disabled by default and never changes optimizer selection.
- **Community showcase** - Added sixteen community showcase images grouped into six full-width project cards, one per row: a finished multicolor King of Hearts with its slicer preview, two credited Hobbits and Dragons views with a source link, Hope poster predictions and results documenting incomplete color matches, credited Titan and Batmanga prediction/slicer/finished-print trios, and a Naruto overnight-print trio. Related photos appear together with individual captions and full-size image links, stacking on small screens. Each project shows its creator, available dimensions, and source action, followed by a GitHub pull-request invitation for new showcases and contributions.
...
- **HD-guided Stack Matrix calibration** - Added a third calibration tab for measuring many fixed-depth physical recipes at once. Choose 2–8 profile filaments, recipe depth, board capacity, and opaque backing; Kromacut prints every combination that fits or uses the existing per-channel HD model to retain a deterministic, color-diverse subset. Planning and slicer-safe 3MF construction run in a worker, use the real profile filaments, include orientation markers and embedded settings, and preserve the immutable recipe plan for later sampling.
  Optical fitting uses robust sRGB observation error with linear-light blending and training-supported mean prior penalties, so dark-channel evidence is represented and unused spools do not change the fit. An identical measured recipe can supply an explicitly estimated correction across a different deeper backing only when the immediate substrate matches and its simulated final color is within 1 ΔE00. Measured prefixes can carry that correction forward through additional layers of the same terminal material, attenuated through the original nonlinear run and fading out within one recipe depth. Both uses retain source provenance and reduced confidence; neither is labeled an exact final-surface observation. Saved calibration and raw measurements remain unchanged.
  ...
  Compatible completed matrices contribute to a shared empirical LUT and a regularized effective optical model while leaving saved HD measurements and raw samples unchanged. Effective-optics corrections are validated by holding out whole matrices when available, or grouped recipe/substrate interactions for a single matrix, and weak or non-improving evidence retains the existing optical prior. Exact photographed recipes and bounded interpolation can override simulation only for the complete measured foundation or a sufficiently opaque, optically equivalent substrate with the same top-material interaction. Candidate scoring, recipe comparisons, and preview share one actual-prefix predictor: fitted interactions stop at the thickest measured contiguous run, then additional thickness continues smoothly toward the nominal filament color using conservative substrate-aware optics. Interpolated corrections fade at the color-coverage boundary, and conditional LUT validation uses the same neighbor selection and fade as runtime. Dead-on Palette Proof evidence remains higher priority, matrix cells are never treated as artwork targets, and physical filament assignments and export colors remain unchanged. Planned work survives restarts, completed matrices can be reprocessed or deleted, planned and completed records have separate retention limits, and profile import/export preserves validated evidence without allowing a same-ID legacy file to erase newer measurements. Switching profiles reselects only the new profile's matrices and cancels stale generation work before it can cross profile ownership.
  New matrices take their regular and first-layer heights from the current 3D print settings, not a stale saved wedge-calibration default. Existing plans, re-downloads, and measurements retain their original print grid.
  New and saved Matrix 3MF exports use the shared desktop Save As dialog, with normal downloads retained in browsers. Cancelled or failed saves leave existing records unchanged and do not register a new plan.
...
- **Preserve color separation (Auto-paint)** - Enhanced matching can require every distinct 2D image color to map to its own printable surface color within a saved hard ΔE limit from 1 to 100 (default 6). Assignment and variable-length stack selection are solved together: candidate comparison prioritizes the most unique in-limit matches and greatest source-image coverage, then fewer repeated appearances, used filament runs, and physical layers before lower in-limit error. This prevents rejected source colors or imperceptible numerical gains from retaining unnecessary transition runs. Search starts without repeats, admits one additional appearance at a time up to the shared **Total repeat limit**, stops early only after complete preservation, and otherwise keeps the best fully searched tier. A final fully simulated deletion pass removes dispensable occurrences and verifies when no single run can be removed. Results distinguish a proven **Exact optimum** from a heuristic **Best found** result instead of treating convergence as proof. **Require a unique match for every color** rejects incomplete results with guidance; when disabled, unmatched colors are dropped as distinct outputs and merged into surviving preserved mappings instead of receiving above-limit fallback colors. The summary reports preserved and merged colors, used and available printable-color capacity, worst preserved ΔE, and actual repeats used; partial results are labeled **Partial palette**. Results with no in-limit survivor, strict failures, and pending calculations cannot build, export, or generate misleading instructions. Preserve color separation remains mutually exclusive with Height dithering because both change the printable height map.
  Near-identical printable-color groups retain every physical candidate when checking the hard limit, so an out-of-limit representative cannot hide another member's valid match. Dropped targets reuse the actual preserved member, and the 3D view keeps the verified final mappings instead of replacing them with rounded-swatch recomputations.
...
- **Auto-paint regression and performance coverage** - Added deterministic stack goldens, layer-invariant and realized CIEDE2000 quality-budget tests, a schema-v2 frontlit-profile import fixture, and reproducible real-world Exact and Thorough benchmarks across four images with accumulated Palette Proof and Stack Matrix evidence. Regression coverage also checks actual-prefix predictor agreement, continuous fit and interpolation boundaries, protected foundations, feasible grouped matches, conditional Matrix validation, and inspection-mode entry. Quality budgets use eligible surfaces and full-precision final predictions. Benchmarks record timing and memory while verifying score, filament order, layer count, separation results, and final-stack fingerprints remain unchanged, and support fixed-seed report-only A/B runs with realized mean, p95, and Delta E 6 coverage.
...
- **3D inspection view modes** - The preview toolbar now offers Color accurate, Shaded, Transparent, and Wireframe modes for inspecting exact selected colors, lit surfaces, layers, and mesh edges. Color accurate bypasses scene lighting and filmic tone mapping so flat surface pixels retain their Simulated or Physical swatch color, including on first entry from Transparent or Wireframe without inheriting opacity or disabled color writes; Wireframe uses thin, layer-colored feature edges, and the remembered selection never changes print settings or exports.
...
- **Auto-paint printable color mapping** - Enhanced matching now keeps its fast continuous candidate generation but reranks a bounded printable-layer neighborhood plus the most promising flat zones with realized CIEDE2000 and prediction confidence. Preserve color separation uses the same realized metric without changing its count, source-weight, repeat, stack-complexity, and in-limit-error priorities, and final preview target heights reuse the optimizer's selected printable layers.
...
- **Frontlit filament calibration** - Replaced photo sampling with a camera-free wedge workflow. Choose one or more filaments and their backing layers, download STL or multi-material 3MF patches, and record when each printed patch first matches its opaque reference rail. Quick mode measures scalar HD and anchors a conservative RGB estimate; optional Accurate reads refine one bounded channel-selectivity curve only as much as their quantized intervals require, without requiring three or four bases or claiming an underdetermined three-channel measurement. Refined selectivity stays local to bases actually tested, merge reads validate the curve and affect confidence, and sufficiently informative sessions can fit a shared JND. The preview and confidence use the exact layer height, base filaments, and settings frozen into the downloaded wedge, and partial entry saves completed filaments without changing unfinished ones. Editing a calibrated swatch temporarily falls back to a color-estimated HD without deleting the measurement; reverting the color restores it. Legacy free-channel wedge records load unchanged but resolve through the constrained compatibility model; legacy photo measurements are removed on load, so older photo-calibrated profiles should be recalibrated.
...
- **Auto-paint optical model** - Beer-Lambert blending now runs in linear-light sRGB, and calibrated filaments use substrate-aware per-channel HD estimates for blend simulation and transition-zone thickness. Unsupported wedge bases use the conservative Quick curve, while fully independent RGB HDs, nonlinear exponents, and substrate interactions come only from held-out-validated Stack Matrix evidence within its measured thickness support. Rebuild existing Auto-paint results before printing because the safer model can change stack heights and swap plans.
...
- **Desktop filament profile export** - `.kfil` exports now open the shared Save As dialog instead of starting a browser-style download. Cancelled saves do not report success, failed saves show an error, and profile contents and calibration remain unchanged. Browser downloads are preserved.
...
- **Auto-paint Max Height** - Planning, scoring, preview, and export now use the same layer-aligned stack. Height caps round down to a printable boundary instead of exceeding the requested maximum with one final layer. Compression, trimming, target mapping, height dithering, and Palette Proof candidates preserve the foundation's layer-snapped opacity minimum; impossible foundations are rejected rather than displayed as opaque after thinning.
...
- **Calibration 3MF XML safety** - Stack Matrix and filament-calibration exports replace XML 1.0-invalid characters in imported filament names, preventing malformed project files that conforming slicers cannot open.
