...
- **HueForge spool CSV/TSV import** - Auto-paint can import HueForge spool-library CSV or TSV files with flexible column order, RFC 4180 quoted fields, embedded commas or newlines, and an optional UTF-8 byte-order mark. Each spool retains its HueForge UUID and imports with its brand, color name, and hex value.
...
- **Saved 3D Auto-paint settings** - Stored settings are applied before the first persistence pass instead of being briefly replaced by defaults during startup or hot reload. Max Height and calibration layer height are remembered with the other options.
...
- **Filament profile validation** - Imported `.kfil` and legacy `.kapp` filaments reject zero or negative hiding distances before invalid values reach the UI.
- **Filament profile imports** - Failed imports no longer report success or replace the in-memory profile.
