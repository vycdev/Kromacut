...
- **Auto-paint printable color mapping** - Enhanced matching now keeps its fast continuous candidate generation but reranks a bounded printable-layer neighborhood with realized CIEDE2000 and prediction confidence. Preserve color separation uses the same realized metric without changing its count, source-weight, repeat, stack-complexity, and in-limit-error priorities, and final preview target heights reuse the optimizer's selected printable layers.
