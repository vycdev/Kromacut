...
- **Palette Proof export** - Auto-paint results now retain one immutable, serializable snapshot of the exact printable physical layers, compressed zones, swap runs, simulated prefix colors, and processed-target mappings used by preview. The calibration dialog includes a Palette Proof tab with a deterministic target-column map and downloadable 3MF using up to five unique reachable prefixes per target, 8 mm patches, 1 mm trenches, 2 mm margins, calibration-matched 1.2 mm rounded corners, a 2 mm orientation notch, one continuous first-layer foundation, physical layer/filament assignments, embedded frozen instructions and patch metadata, stale/non-prefix validation, and manifold topology regression coverage. Physical comparison feedback does not modify the appearance model yet.
