...
- **Palette Proof export and results** - Auto-paint results now retain one immutable, serializable snapshot of the exact printable physical layers, compressed zones, swap runs, simulated prefix colors, and processed-target mappings used by preview. The calibration dialog includes a Palette Proof tab with selectable target and candidate counts, a deterministic target-row map shared by the proof and Results views, and downloadable 3MF using up to five unique reachable prefixes per target, 8 mm patches, selectable 1 mm or touching spacing, 2 mm margins, calibration-matched 1.2 mm rounded corners, a 2 mm orientation notch, one continuous first-layer foundation, sample-safe reinforcement, physical layer/filament assignments, embedded frozen instructions and patch metadata, stale/non-prefix validation, and manifold topology regression coverage. Experimental touching proofs sort candidates by stack height and union adjacent cells per layer to reduce slicer islands and travel; because no interior trench remains, they omit the extra reinforcement grid and rely on the connected candidate layers above the foundation. Exported proofs persist in the active named filament profile with their exact process and prefix data; incomplete jobs and draft results can be deleted, while completed evidence is protected. The Results view records one closest patch, multiple tied patches, or no match for every target, saves progress immediately across restarts, and locks or reopens completed evaluations. Completed jobs can start a non-duplicative next proof that anchors one prior best, prioritizes untested targets, and spends remaining rows on untested prefixes. Profile schema v3 exports/imports this bounded, validated appearance evidence without changing v2 hiding-distance semantics. Physical comparison feedback remains evidence only and does not modify the appearance model yet.
