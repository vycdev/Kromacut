...
| **Preserve color separation** | (Requires Enhanced color matching) Globally evaluates unique printable-color assignments within a configurable ΔE limit. Strict mode rejects a result when every 2D color cannot receive its own acceptable printable color; fallback mode keeps the result and explicitly reports color reuse, printable-color capacity, and any final mappings above the limit. Mutually exclusive with Height dithering. |
