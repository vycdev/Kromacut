...
Choose **Download 3MF** to generate the coupon. The default 8-target, 5-candidate layout is 48 x 75 mm; reducing either count makes a smaller proof. Each target occupies one row and its A-E candidates run left to right, matching the Proof map and Results views. It uses 8 mm patches, 1 mm trenches, 2 mm margins, 1.2 mm rounded corners, and a 2 mm missing top-left corner for orientation. The first physical layer is one continuous foundation. Two later layers reinforce the margins and trenches while leaving clearance around every sample square, so the coupon is stiffer without changing the stack being judged. Candidate regions continue only to their selected prefixes.

Use **Spacing > Touching** to generate an experimental no-gap coupon. At the default counts it is 44 x 68 mm. Kromacut orders each row from the lowest to highest candidate prefix and unions adjacent cells on every physical layer, reducing the number of separate slicer regions and potential travel moves. Because there are no trenches, this version omits the extra reinforcement grid and relies on the connected candidate layers above the foundation; candidate boundaries may also be less visually distinct. Keep the 1 mm version as the reference while comparing print time and handling.
