...
After Auto-paint produces at least two printable stack prefixes, open **Calibrate Filaments** and select the **Palette Proof** tab. Choose how many image **Targets** and physical stack **Candidates** to compare. Each target row shows the desired image color followed by candidates `A` through `E`. A dashed `F` cell points to the shared foundation margin instead of creating a duplicate first-layer patch.
