...
The planned record is saved when the 3MF is generated, so you can close Kromacut while the print runs and resume later. Kromacut retains planned and completed records in separate bounded groups, so creating a new plan does not evict completed calibration. A completed record can be photographed again to replace its measurements, downloaded again, or deleted. Only the newest completed matrix compatible with the current profile and layer height supplies matrix anchors. A matrix made from a selected subset of the profile remains compatible with its unchanged full owner profile.
