...
After Auto-paint produces at least two printable stack prefixes, open **Calibrate Filaments** and select the **Palette Proof** tab. Choose how many image **Targets** and physical stack **Candidates** to compare. Under **Prioritize image colors**, pin processed-palette swatches that matter most for the current artwork, such as skin tones; the percentage on each swatch shows how much of the image uses it. Kromacut keeps pinned colors in the proof and automatically fills the remaining target slots with useful high-coverage and color-diverse choices. Leave every swatch unpinned for fully automatic selection. Each target row shows the desired image color followed by candidates `A` through `E`. A dashed `F` cell points to the shared foundation margin instead of creating a duplicate first-layer patch.
...
The 3MF contains one part per physical layer with the final Auto-paint filament assignments. It also embeds `palette-proof.json` and `palette-proof-instructions.txt` under its `Metadata` folder so the printed coupon stays tied to the exact stack snapshot and patch IDs that generated it. A saved proof can be downloaded again while its exact source Auto-paint result is active. Keep the model face-up at 100% scale, use the layer heights shown in the instructions, confirm filament assignments in the slicer, and compare the printed candidates under your normal viewing light.

Saving the 3MF also records the proof in the active named filament profile. Open the **Results** view after printing and, for each target row, select the visibly closest patch even when it is imperfect, select multiple patches when they are tied, or choose **None** only when every candidate is clearly a poor match. Progress is saved immediately, survives app restarts, and can be completed and reopened for correction. The proof selector groups records that contain the same target colors into numbered target sets and labels later rounds as continuations, including existing records created before grouping was added. When the same image and compatible print settings are active, the automatic **Next proof** uses all completed history to prioritize untested and least-tested targets instead of returning to the first target set. After completion, choose **Continue targets** to keep the exact target colors and test untried stack candidates around their prior bests. Choose **New targets** when you want to explicitly branch from a selected completed proof and prioritize colors outside it; it produces a fully new target set when enough untested colors remain and otherwise fills the sheet with the least-tested prior targets. Each action is disabled when it has no relevant untested evidence. The trash button deletes either an incomplete or completed proof together with all of its results, removing that evidence from appearance calibration. Saved proofs remain available from the profile even when the original image is no longer loaded.
