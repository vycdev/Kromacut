...
- **Preserve color separation** tries to give every distinct 2D image color its own printable surface color within the selected **Unique-match limit**. The numeric field accepts ΔE 1 to 100 and defaults to 6; raising it makes preserving every color more feasible, but permits less accurate matches. Kromacut evaluates unique, in-limit assignments globally so one dominant region cannot take the only good match from another color. It first searches at the selected optimizer effort without repeating a filament, then permits one additional filament occurrence at a time and stops at the first successful complexity level. **Require a unique match for every color** is enabled by default and rejects a result that cannot satisfy every color. Turn it off to keep the result: remaining source colors use best-supported printable fallbacks and may merge. A fallback may be within the selected limit but reuse a printable color, or it may exceed the limit; the result reports both cases explicitly.
...
The total repeat limit is a ceiling, not a target. With Preserve color separation enabled, the completed result separately reports unique in-limit assignments, source colors that reuse printable colors, the number of distinct printable colors available, final mappings above the limit, and the worst final mapped ΔE. A worst mapped value above the Unique-match limit is therefore possible when strict failure is off: the limit governs whether every source color can receive its own acceptable printable color, while fallback mappings are still shown even when they exceed it. To resolve a strict separation failure, raise the Unique-match limit if less accurate matches are acceptable, increase Max Height or the repeat limit, add a suitable filament, reduce the processed 2D palette, or turn off **Require a unique match for every color** to permit explicit fallback mappings. A rejected Auto-paint calculation cannot be built or exported and never falls back to the unrelated manual color-height stack; the preview may continue showing the previous valid build.
