...
4. **Enter Results** by reporting, for each filament/base row, the **first patch that looks identical to the reference rail** beside it. One read measures scalar HD and anchors a conservative RGB-channel estimate. Additional base reads refine a bounded channel-selectivity curve only as much as the observed patch intervals require; they do not freely solve three independent channel values. When the session has enough informative data it may also fit the shared JND. The optional merge field records where adjacent steps stop looking different, validates the curve, and can lower confidence when it disagrees without blocking saving.
...
Because the reference rail is the filament's own fully-opaque color, you are only judging whether a patch matches the rail right next to it — a comparison that stays reliable across lighting and screens. The measured scalar hiding distance transfers to your real prints. A stronger channel-selectivity refinement from extra bases is used directly on those measured bases; an untested base keeps the conservative Quick estimate unless Stack Matrix evidence supports that material pair and thickness.
...
- **Preserve color separation** tries to give every distinct 2D image color its own printable surface color within the selected **Unique-match limit**. The numeric field accepts ΔE 1 to 100 and defaults to 6; it is a hard eligibility boundary, so a printable color above the limit cannot preserve that source color as a distinct output. Kromacut evaluates unique assignments and variable-length physical stacks together. It first maximizes uniquely preserved colors and source-image coverage, then prefers fewer repeated appearances, fewer used filament runs, and fewer physical layers before using lower in-limit error as a tie-breaker. This prevents a tiny numerical improvement from retaining a large transition stack. Search begins without repeats, permits one additional occurrence at a time, and stops early only when every source color is preserved; otherwise it exhausts the selected repeat budget and keeps the best tier. **Require a unique match for every color** is enabled by default and rejects an incomplete result. Turn it off to build a partial palette: unmatched source colors are dropped as distinct colors and their regions merge into the nearest surviving preserved mapping. The chosen sequence is then checked by removing and fully recomputing each occurrence until no single run can be removed without worsening those same priorities.
...
The total repeat limit is a ceiling, not a target. With Preserve color separation enabled, the completed result reports unique in-limit assignments, dropped-and-merged source colors, printable-color capacity, and the worst preserved ΔE. That worst preserved value cannot exceed the Unique-match limit. A merged source color can be visually less accurate because it no longer owns a distinct printable output; it is not counted as an above-limit success. To resolve a strict separation failure, raise the Unique-match limit if less accurate matches are acceptable, increase Max Height or the repeat limit, add a suitable filament, reduce the processed 2D palette, or turn off **Require a unique match for every color** to permit dropped-color merging. If no source color can be preserved inside the limit, Kromacut rejects the calculation even in non-strict mode because there is no valid mapping to merge into. A rejected Auto-paint calculation cannot be built or exported and never falls back to the unrelated manual color-height stack; the preview may continue showing the previous valid build.
...
The result badge distinguishes **Exact optimum** from **Best found**. Exact optimum means every
candidate in the applicable no-repeat search space was compared. Repeat-enabled refinement and the
Fast, Balanced, Thorough, and Deep tiers are heuristic and therefore report Best found. **No
removable run** is a narrower guarantee: every one-run deletion was fully recomputed and none could
preserve the same colors and coverage with a simpler accepted stack; it does not claim that a
different multi-run rearrangement cannot be better.
...
- **Optimizer Performance**, including Algorithm, Quality Score, Iterations, Cache hit, **Exact optimum** or **Best found**, and the **No removable run** verification when available. Quality Score is shown for complete separation results. A non-strict result that merges unmatched colors is labeled **Partial palette** instead of displaying the optimizer's internal search score as a meaningful perceptual-quality value.

Prediction confidence is distinct from raw color error. Kromacut lowers it as a recipe moves away from physical measurements, nearby measured corrections disagree, or held-out predictions miss their photographed colors. Exact measurements start strongest, followed by local interpolation, fitted models, and unsupported simulation. Enhanced matching adds a bounded uncertainty cost to ordinary color optimization. Preserve color separation evaluates eligibility and preserved-match ranking against raw ΔE, so uncertainty cannot make an out-of-limit color valid. In non-strict mode, any such source color is merged into a surviving in-limit mapping rather than retained as a distinct above-limit output.
...
New matrices use the current **Layer Height** and **First Layer Height** from the 3D print settings. Set those values before creating the board; changing the Hiding Distance wedge's layer height does not change the matrix. Once generated, each saved matrix keeps its original layer heights, recipes, and measurements, even if you later change your print settings or download that matrix again.

On desktop, both **Create and download 3MF** and a saved matrix's **Download 3MF** open a **Save As** dialog so you can choose the file name and location. Cancelling leaves saved matrices unchanged and does not add a new plan. In the browser, these buttons use the normal browser download flow.
...
The planned record is saved after the 3MF is successfully saved on desktop or its browser download is started, so you can close Kromacut while the print runs and resume later. Kromacut retains planned and completed records in separate bounded groups, so creating a new plan does not evict completed calibration. A completed record can be photographed again to replace its measurements, downloaded again, or deleted. Every stored completed matrix compatible with the current profile and layer height supplies empirical LUT evidence and exact matrix anchors. A matrix made from a selected subset of the profile remains compatible with its unchanged full owner profile. Compatible samples also jointly refine effective RGB-channel HD, opaque filament appearance, nonlinear transmission through repeated layers, and ordered filament-over-substrate behavior. The existing filament colors and HD measurements are conservative priors and are never overwritten; Kromacut applies the derived physical fit only when there is enough evidence and it improves the matrix prediction.
