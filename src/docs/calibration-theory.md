...
A completed matrix stores predicted and photographed sRGB colors beside the immutable physical recipes in the named filament profile. The newest compatible matrix becomes a scattered empirical LUT in predicted Lab and physical recipe space. An exact fixed-depth layer recipe uses its photographed Lab value directly. A missing recipe uses a deterministic inverse-distance interpolation of nearby photographed Lab values, with physical layer order weighted toward the optically dominant top layers. Interpolation is allowed only inside local predicted-Lab coverage and a bounded recipe neighborhood; otherwise the Beer-Lambert/HD result remains unchanged. This avoids unsupported extrapolation while letting photographed combinations correct both optimizer scoring and the final preview. Dead-on Palette Proof anchors still take priority over matrix evidence, and matrix cells are observations rather than desired image targets, so printing a broad matrix does not make the optimizer chase every sampled color.
