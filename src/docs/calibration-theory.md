...
A completed matrix stores predicted and photographed sRGB colors beside the immutable physical recipes in the named filament profile. All stored compatible matrices jointly refit one effective physical model while keeping the saved swatch colors, HD calibration, and raw matrix measurements untouched. The fit treats those saved values as regularized priors, then uses every weighted matrix sample to estimate effective RGB-channel HD, effective opaque filament color, a nonlinear transmission exponent for contiguous runs of one filament, and an ordered interaction between the visible filament and the substrate below it. Sparse evidence stays on the original priors, and a fitted model is used only when there are enough samples and it improves held-out matrix ΔE without worsening its error tail. A fitted substrate interaction is used through the thickest contiguous run of that material pair observed by the matrices. Additional unmeasured thickness continues from that supported color using the substrate-appropriate wedge/HD prior toward the filament's nominal swatch; it does not switch the entire run to a different prediction or extrapolate the fitted exponent indefinitely. The same run-prefix calculation feeds candidate scoring, recipe comparisons, and the preview, including compressed and layer-snapped stacks.

The fit compares its simulated colors with the photographed sRGB values using a robust error measure, while the physical blending still happens in linear light. This gives differences in dark channels enough weight to influence the fit. The prior penalties are averaged by parameter family over the materials actually represented in the training samples; adding unused spools does not weaken or disable the same fit. Validation still determines whether the fitted model is used. Better agreement on familiar recipes does not establish accuracy for previously unmeasured filament pairs.

The compatible matrices also remain scattered empirical LUTs in predicted Lab and physical recipe space, so a newer sparse board does not erase recipes measured by an earlier board. Kromacut weights each board by reviewed alignment confidence, measured recipe coverage, recency, and robust agreement with recipes measured by at least two other boards. With only one or two observations of a recipe, agreement remains neutral because there is not enough evidence to identify an outlier. An exact fixed-depth layer recipe combines its photographed Lab observations directly. A missing recipe combines deterministic inverse-distance interpolations of nearby photographed Lab values, with physical layer order weighted toward the optically dominant top layers. Interpolation is allowed only inside each matrix's local predicted-Lab coverage and a bounded recipe neighborhood; otherwise the jointly fitted physical model is used. Outside compatible matrix evidence, that model falls back to the saved Beer-Lambert/HD priors. Both optimizer scoring and the final preview share the same prediction. Dead-on Palette Proof anchors still take priority over matrix evidence, and matrix cells are observations rather than desired image targets, so printing a broad matrix does not make the optimizer chase every sampled color.

## Prediction Uncertainty

Inside the Matrix's local predicted-color coverage, interpolated corrections fade smoothly back toward the physical model as their support runs out. Exact photographed recipes keep their measured colors. A matching top material alone is not enough to transfer a Matrix measurement: the full measured foundation must match, or the alternative backing must be sufficiently opaque and optically equivalent with the same immediate substrate interaction.

For an identical measured recipe, Kromacut can also estimate a transfer to a different deeper backing when the immediate substrate is the same material and the simulated final colors differ by at most 1 ΔE00. The starting foundation must meet the opacity floor from either the current fit or the saved HD prior; diagnostics record which supported its thickness. This lets an existing print retain the prior that justified its base when a later fit changes the estimated floor. The measured correction transfers with reduced confidence and is labeled **interpolated**, since backing equivalence is inferred from the model rather than independently measured. This exception does not enable general recipe interpolation on a different backing.

Adding more of the same terminal filament can carry a measured prefix's correction forward. Its influence is attenuated through the original physical run and fades out over at most one Matrix recipe's thickness of extra material. It is also labeled interpolated. It does not predict thinner prefixes from a later measurement, and adding a different material ends this continuation. Direct measurements and applicable Palette Proof judgments retain their priority. Diagnostic records identify the source samples and whether evidence was transferred across a backing or continued through added thickness.
...
- **Validation error:** the LUT predicts each recipe without its own empirical sample, using the same neighbor selection and support fade as runtime. This is a conditional check with the fitted optical model held fixed, not an independent end-to-end accuracy estimate. Separately, the optical fit is refitted and tested using whole held-out matrices, or grouped recipe/substrate interactions when only one matrix is available.
...
- **One base read (Quick mode):** the opacity comparison measures scalar HD. RGB-channel differences remain a conservative estimate from the filament's swatch color, anchored so the brightest channel matches that measurement.
- **Additional base reads (Accurate mode):** each base stresses the estimated channel curve differently. Kromacut adjusts one bounded channel-selectivity strength and the scalar only as much as the quantized patch intervals require. It does not claim that two visual thresholds independently measured three spectral HD values.

The refined curve is used directly for base materials that were actually compared in the wedge. On an untested base, Kromacut keeps the Quick estimate instead of extrapolating a strong hue shift. You may add more bases when convenient, but three or four are not required. Fully independent channel HDs, nonlinear transmission, and substrate-specific interactions are reserved for Stack Matrix evidence with held-out and thickness-support checks.

The optional merge read records the last adjacent wedge step that still looked different. It validates the fitted curve rather than adding another free parameter: a large mismatch lowers confidence and is shown in diagnostics.

## The Session JND Fit

The default JND of 2 ΔE00 is a human-vision constant, but a given viewer and lighting may sit slightly off it. When a session has enough independently informative multi-base reads, Kromacut can fit one shared JND from 1 to 3. The fit is kept only when it is clearly identifiable and beats the default; quantized wedge reads are often ambiguous with scalar HD, in which case the session correctly keeps the constant.
